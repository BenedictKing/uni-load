  ...
  private async cleanupInstanceTempGroups(instance: any): Promise<{
    ...
    try {
      ...
      const tempGroups = groups.filter(
        (group) => group.name.startsWith('temp-test-') || group.name.startsWith('debug-models-')
      )
      ...
      try {
        ...
        if (groups && Array.isArray(groups)) {
          const tempGroups = groups
            .filter((group) => group.name.startsWith('temp-test-') || group.name.startsWith('debug-models-'))
            .map((group) => ({
              id: group.id,
              name: group.name,
