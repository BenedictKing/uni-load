...
class GptloadService {
  ...
  async getStatus() {
    try {
      ...
      const healthyCount = Object.values(instances).filter(
        (inst) => inst.healthy
      ).length;
  ...
  async toggleApiKeysStatusForGroup(groupId, instanceId, status) {
    ...
    return await this.manager.toggleApiKeysStatusForGroup(
      instance,
      groupId,
      status
    );
  ...
  async deleteAllModelGroups() {
    ...
    for (const group of modelGroupsToDelete) {
      try {
        const success = await this.deleteGroupById(
          group.id,
          group._instance.id
        );
        if (success) {
    ...
    console.log(
      `🏁 批量删除完成: 成功 ${results.deleted.length} 个, 失败 ${results.failed.length} 个`
    );
    return results;
  }
  ...
  async createOrUpdateModelGroups(models, siteGroups) {
    ...
    console.log(
      `✅ 已获取 ${allExistingGroups.length} 个分组，开始处理模型...`
    );

    for (const model of models) {
      try {
        // 将预加载的分组列表传递下去
        const modelGroup = await this.createOrUpdateModelGroup(
          model,
          siteGroups,
          allExistingGroups
        );
   ...
   * 创建或更新单个模型分组
   */
  async createOrUpdateModelGroup(
    originalModelName,
    siteGroups,
    allExistingGroups
  ) {
    ...
    const isPreferredTypeAvailable = siteGroups.some(
      (sg) => sg.channel_type === preferredChannelType
    );
    ...
    if (isPreferredTypeAvailable) {
      ...
      console.log(
        `✅ 找到匹配的模型原生格式 [${preferredChannelType.toUpperCase()}] 的站点分组`
      );
    } else {
      // 否则，回退到第一个可用的站点分组格式
    ...
    const groupName = this.generateSafeGroupName(
      originalModelName,
      channelType
    );
    ...
    console.log(
      `处理模型: ${originalModelName} (格式: ${channelType}) -> 分组名: ${groupName}`
    );

    // 优化：检查模型分组是否已存在（从预加载的列表中查找）
    const existingGroup = allExistingGroups.find(
    ...
    if (!localHealth?.healthy) {
      ...
      const healthyInstanceId = Object.keys(allInstances).find(
        (id) => allInstances[id].healthy
      );
    ...
    try {
      ...
      const channelConfig = this.getChannelConfig(channelType);
      console.log(
        `ℹ️ 模型 ${originalModelName} 将使用 ${channelType.toUpperCase()} 格式`
      );
      ...
      const response = await targetInstance.apiClient.post(
        "/groups",
        groupData
      );
      ...
      if (targetInstance.token) {
        try {
          await this.manager.addApiKeysToGroup(targetInstance, group.id, [
            targetInstance.token,
          ]);
          console.log(`✅ 已为模型分组 ${groupName} 添加gpt-load访问token`);
        } catch (error) {
          console.warn(
            `⚠️ 为模型分组添加gpt-load token失败: ${error.message}，但分组已创建成功`
          );
        }
      } else {
        console.warn(
          `⚠️ 实例 ${targetInstance.name} 没有配置token，模型分组 ${groupName} 将无API密钥`
        );
      }

      console.log(
        `✅ 模型分组 ${groupName} 创建成功，包含 ${upstreams.length} 个上游 (实例: ${targetInstance.name})`
      );
  ...
  async addSiteGroupsToModelGroup(modelGroup, siteGroups) {
    ...
    try {
      ...
      for (const siteGroup of siteGroups) {
        ...
        // 检查是否已经包含此上游
        const existingUpstream = currentUpstreams.find(
          (upstream) => upstream.url === newUpstreamUrl
        );
        ...
        if (!existingUpstream) {
          ...
          addedCount++;
          console.log(
            `➕ 添加站点分组 ${siteGroup.name} 到模型分组 ${modelGroup.name} (跨实例)`
          );
        } else {
          console.log(
            `⚡ 站点分组 ${siteGroup.name} 已存在于模型分组 ${modelGroup.name}`
          );
        }
      }
      ...
      if (addedCount > 0) {
        ...
        console.log(
          `✅ 已添加 ${addedCount} 个站点分组到模型分组 ${modelGroup.name} (实例: ${instance.name})`
        );
      } else {
        console.log(
          `ℹ️ 模型分组 ${modelGroup.name} 无需更新，所有站点分组已存在`
        );
      }
  ...
  async analyzeChannelHealth(groupName, instanceId, timeRangeHours = 24) {
    const logs = await this.getChannelLogs(
      groupName,
      instanceId,
      timeRangeHours
    );
    ...
    failedLogs.forEach((log) => {
      const errorKey = `${log.status_code}_${
        log.error_message?.substring(0, 50) || "unknown"
      }`;
  ...
  async deleteChannelCompletely(channelName) {
    ...
    try {
      await this.deleteGroupById(
        siteGroupToDelete.id,
        siteGroupToDelete._instance.id
      );
    ...
    console.log(
      `🔍 发现 ${modelGroupsToUpdate.length} 个模型分组引用了该渠道，开始清理...`
    );

    for (const modelGroup of modelGroupsToUpdate) {
      try {
        const newUpstreams = modelGroup.upstreams.filter(
          (u) => !u.url.includes(upstreamToRemove)
        );
