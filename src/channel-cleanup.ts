...
class ChannelCleanupService {
  ...
  async cleanupDisconnectedChannels(options = {}) {
    ...
    try {
      ...
      // 3. 检测不可连接的站点分组
      const disconnectedChannels = await this.detectDisconnectedChannels(
        siteGroups, 
        { timeout, retryCount, excludePatterns, onlyCheckPatterns }
      );
      ...
      // 4. 从模型分组中清理这些渠道的上游
      const cleanupResults = await this.cleanupUpstreamsFromModelGroups(
        modelGroups, 
        disconnectedChannels, 
        dryRun
      );
      ...
      console.log(`📈 统计：清理了 ${results.cleanedUpstreams} 个上游，影响 ${results.affectedModelGroups.length} 个模型分组`);
  ...
  isSiteGroup(group) {
    ...
    const hasExternalUpstream = group.upstreams.some(upstream => 
      upstream.url && !upstream.url.includes('/proxy/')
    );
  ...
  isModelGroup(group) {
    ...
    const hasProxyUpstream = group.upstreams.some(upstream => 
      upstream.url && upstream.url.includes('/proxy/')
    );
  ...
  async cleanupUpstreamsFromModelGroups(modelGroups, disconnectedChannels, dryRun) {
    ...
    for (const modelGroup of modelGroups) {
      const cleanupResult = await this.cleanupUpstreamsFromSingleGroup(
        modelGroup, 
        upstreamPatternsToRemove, 
        dryRun
      );
  ...
  async cleanupUpstreamsFromSingleGroup(modelGroup, patternsToRemove, dryRun) {
    ...
    for (const upstream of currentUpstreams) {
      const shouldRemove = patternsToRemove.some(pattern => 
        upstream.url && upstream.url.includes(pattern)
      );
    ...
    console.log(`${dryRun ? '📋' : '✅'} 模型分组 ${modelGroup.name}: ${dryRun ? '计划' : '已'}移除 ${removedUpstreams.length} 个上游`);
  ...
  async manualCleanupChannels(channelNames, dryRun = false) {
    ...
    // 找到指定的渠道分组
    const targetChannels = siteGroups.filter(group => 
      channelNames.includes(group.name)
    );
