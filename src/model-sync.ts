...
class ModelSyncService {
      ...
      for (const siteGroup of siteGroups) {
        try {
          ...
          if (syncResult.hasChanges) {
            totalSynced++
            console.log(
              `✅ ${siteGroup.name}: 同步了 ${syncResult.changes.added.length} 个新模型，移除了 ${syncResult.changes.removed.length} 个模型`
            )
          } else {
  ...
  getConfiguredModels(siteName, allGroupsCache) {
    ...
    const modelGroups = allGroupsCache.filter((group) => {
      ...
      return group.upstreams?.some((upstream) => upstream.url.includes(`/proxy/`) && upstream.url.includes(siteName))
    })
  ...
  async applyModelChanges(siteInfo, changes, allGroupsCache) {
    ...
    if (changes.added.length > 0) {
      ...
      // 优化：从缓存中获取该站点的所有格式分组
      const siteGroups = allGroupsCache.filter(
        (group) => group.name.startsWith(siteName + '-') && this.filterSiteGroups([group]).length > 0
      )
  ...
  getStatus() {
    return {
      ...
      nextSync: this.syncInterval ? new Date(Date.now() + this.syncIntervalMinutes * 60 * 1000).toISOString() : null,
    }
  }
}
