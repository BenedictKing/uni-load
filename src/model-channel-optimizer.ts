...
class ModelChannelOptimizer {
  ...
  async ensureModelChannelGroups(model, requiredChannels = null) {
    ...
    const existingChannelNames = new Set(
      existingGroups.map(g => this.extractChannelNameFromGroup(g))
    );
  ...
  async intelligentHealthCheck(model) {
    ...
    console.log(`🎯 模型 ${model} 健康检查完成: ${report.summary.healthy}健康, ${report.summary.degraded}降级, ${report.summary.critical}危急`);
  ...
  async optimizeModelGroups(model, groups) {
    ...
    for (let i = 0; i < groupsWithStats.length; i++) {
      ...
      const actualAdjustment = Math.sign(newPriority - currentPriority) * 
        Math.min(Math.abs(newPriority - currentPriority), maxAdjustment);
