...
uni-load v2.1 是一个基于服务化架构的 AI 站点管理工具，用于自动配置 AI 站点到 [gpt-load](gpt-load.md) 和 uni-api 系统。v2.1 版本经过全面重构，采用依赖注入模式，提供更高的可维护性和稳定性。本手册详细介绍如何使用 Web 界面和 API 接口完成各种操作。
